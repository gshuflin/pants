...
class ClocTest(ConsoleRuleTestBase):
  ...
  def assert_counts(self, result: Sequence[str], lang: str, num_files: int, blank: int, comment: int, code: int):
    for line in result:
      fields = line.split()
      if len(fields) >= 5:
        if fields[0] == lang:
  ...
  def test_cloc(self) -> None:
    ...
    output = self.execute_rule(args=['src/py/foo', 'src/java/foo', '--clocv2-no-transitive']).splitlines()
  ...
  def test_ignored(self) -> None:
    ...
    output = self.execute_rule(args=['src/py/foo', '--clocv2-ignored'])
