...
def run_python_test(test_target, pytest, python_setup, subprocess_encoding_environment):
  ...
  interpreter_constraints = {
    ...
    for constraint in python_setup.compatibility_or_constraints(
      getattr(target.adaptor, 'compatibility', None)
    )
  }
  ...
  directories = tuple(stripped_sources.snapshot.digest for stripped_sources in source_root_stripped_sources)
  sources_digest = yield Get(Digest, DirectoriesToMerge(directories=directories))
